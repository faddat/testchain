...
# docker buildx build --tag testchain --file .pi/Dockerfile --platform linux/arm64 --cache-from testchain:cache --cache-to testchain:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar testchain
...
sudo bash -c "echo testchain > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/testchain.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/testchain.img
